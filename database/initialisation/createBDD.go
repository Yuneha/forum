...
func CreateBDD() error {
	...
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS `Users` (`Id` INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE NOT NULL, `Username` TEXT UNIQUE NOT NULL, `Email` TEXT UNIQUE NOT NULL, `Password` TEXT NOT NULL, `RegistrationDate` TEXT NOT NULL, `Role` INTEGER NOT NULL DEFAULT 0, `Connected` INTEGER NOT NULL DEFAULT 0, `ImagePath` TEXT NOT NULL DEFAULT '/front/static/imgs/Default_pfp.svg', `PromoteRequest` INTEGER NOT NULL DEFAULT 0);")
	if err != nil {
		return err
	}
